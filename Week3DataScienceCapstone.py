...
# ##### It can be observed that Etobicoke has the most number of neighbourhood and the Mississauga has the least
...
# #### Renaming the feature "Post Code" with "Postal Code"

# In[42]:


canada_final = canada_final.rename(columns = {"Postcode": "Postal Code"})
...
# #### Merging the two datasets using inner merge on the feature postal code

# In[44]:


final_df = pd.merge(canada_final, geo_df, on = ["Postal Code"])
